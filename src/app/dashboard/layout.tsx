...
import { LayoutDashboard, Users, ShoppingCart, Settings, Camera, QrCode, ShieldCheck } from "lucide-react";
      ...
      <aside className="w-64 border-r border-zinc-800 bg-[#09090b] flex flex-col relative z-10 shrink-0">
          ...
          <Link href="/dashboard" className="flex items-center gap-3 group">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-semibold mt-1">
                {isMember ? "Espace Membre" : "Enterprise SaaS"}
              </p>
            </div>
          </Link>
          ...
          {gym && (
            <div className="mt-4 p-2.5 rounded-xl bg-zinc-900/80 border border-zinc-800/80">
              ...
              <p className="text-[10px] text-emerald-400 font-medium">
                {isMember ? "MEMBRE ADHÉRENT" : `${gym.subscriptionTier || 'PRO'} TENANT`}
              </p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto">
          {isMember ? (
            <Link 
              href="/dashboard" 
              className="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 text-sm font-semibold transition-colors"
            >
              <QrCode className="w-4 h-4 text-emerald-400" /> Mon Pass & Accès QR
            </Link>
          ) : (
            <>
              <Link 
